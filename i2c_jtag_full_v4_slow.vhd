...
signal ir_value : std_logic_vector(15 downto 0) := (others => '1');
signal ir_data : std_logic_vector(15 downto 0) := (others => '0');

constant JTAG_IDCODE : std_logic_vector(15 downto 0) := x"001E";  -- BScan IDCODE(0x01) + ARM IDCODE(0x0E)
constant JTAG_EXTEST : std_logic_vector(15 downto 0) := x"000F";  -- BScan EXTEST(0x00) + ARM BYPASS(0x0F) - This is OK
constant JTAG_SAMPLE : std_logic_vector(15 downto 0) := x"002F";  -- BScan SAMPLE(0x02) + ARM BYPASS(0x0F) - This is OK
constant JTAG_BYPASS : std_logic_vector(15 downto 0) := x"01FF";  -- Both BYPASS
    ...
    constant JTAG_DIV_MAX : unsigned(7 downto 0) := to_unsigned(15, 8);
	signal tdo_sync : std_logic_vector(1 downto 0) := (others => '0');
  ...
  --  signal ir_value     : std_logic_vector(7 downto 0) := (others => '1');
    signal tdi_data     : std_logic_vector(BSR_LENGTH-1 downto 0) := (others => '0');
--	signal ir_data : std_logic_vector(7 downto 0) := (others => '0');
  ...
  --  signal i2c_sda_in   : std_logic := '1';
	...
	-- I2C pin positions in BSR (confirmed working)
	constant I2C_SDA_OUT  : natural := 15;
	constant I2C_SDA_CTRL : natural := 16;
	constant I2C_SDA_IN   : natural := 14;
	constant I2C_SCL_OUT  : natural := 18;
	constant I2C_SCL_CTRL : natural := 19;
	signal i2c_bit_cnt      : unsigned(3 downto 0) := (others => '0');
	signal i2c_byte_cnt     : unsigned(7 downto 0) := (others => '0');
	signal i2c_reading      : std_logic := '0';
	signal i2c_return_state : main_state_t := IDLE;
	signal i2c_ack_ok : std_logic := '0';
        ...
        if rising_edge(clk) then
            if rst_n = '0' then
                ...
                case state is
                    ...
                    when PROCESS_CMD =>
                        if cmd_byte = x"00" then
                            ...
                            case rx_byte is
                                ...
                                when x"20" =>  -- I2C Write: <addr> <len> <data...>
								if param_cnt = 0 then
									...
									param_cnt <= param_cnt + 1; 
									if unsigned(rx_byte) = 0 then
										-- No data, just address probe
										   i2c_cur_byte <= i2c_addr & '0';  -- FIXED: use stored address
										i2c_bit_cnt <= (others => '0');
										i2c_byte_cnt <= (others => '0');
										state <= I2C_START_1;
										cmd_byte <= x"00";
									else
									...
									-- Store data bytes
									i2c_data_buf(to_integer(param_cnt - 2) * 8 + 7 downto to_integer(param_cnt - 2) * 8) <= rx_byte;
									param_cnt <= param_cnt + 1;
									if param_cnt - 1 >= param_need then
										-- All data received, start I2C
										i2c_cur_byte <= i2c_addr & '0';  -- addr + W
										i2c_bit_cnt <= (others => '0');
										i2c_byte_cnt <= i2c_wr_len;
										state <= I2C_START_1;
										cmd_byte <= x"00";
									else
										state <= IDLE;
                    ...
                    --------------------------------------------------------
					--------------------------------------------------------
					-- I2C BSR Shift (internal - shifts BSR then returns)
					--------------------------------------------------------
					when I2C_BSR_SHIFT =>
						-- Just shift BSR (EXTEST already loaded by C# before I2C command)
						dr_len <= to_unsigned(BSR_LENGTH, 16);
						shift_reg <= bsr_data;
						state <= JTAG_GOTO_SHIFT_DR;
						next_state <= I2C_BSR_WAIT;

					when I2C_BSR_WAIT =>
						-- BSR shift done, capture input and return
						bsr_data <= shift_reg;  -- save captured data
						state <= i2c_return_state;

					--------------------------------------------------------
					-- I2C START: SDA high, SCL high, SDA low, SCL low
					--------------------------------------------------------
					when I2C_START_1 =>
					bsr_data <= (I2C_SDA_CTRL => '1', I2C_SCL_CTRL => '1', others => '0');
					i2c_return_state <= I2C_START_2;
					state <= I2C_BSR_SHIFT;

					when I2C_START_2 =>
						bsr_data(I2C_SDA_CTRL) <= '0';  -- SDA drive
						bsr_data(I2C_SDA_OUT) <= '0';   -- SDA low
						i2c_return_state <= I2C_CLOCK_LOW;
						state <= I2C_BSR_SHIFT;

					--------------------------------------------------------
					-- I2C CLOCK LOW: SCL low, then next bit or done
					--------------------------------------------------------
					when I2C_CLOCK_LOW =>
						bsr_data(I2C_SCL_CTRL) <= '0';  -- SCL drive
						bsr_data(I2C_SCL_OUT) <= '0';   -- SCL low
						i2c_return_state <= I2C_SEND_BIT;
						state <= I2C_BSR_SHIFT;

					--------------------------------------------------------
					-- I2C SEND BIT: setup SDA, then clock high
					--------------------------------------------------------
					when I2C_SEND_BIT =>
						if i2c_bit_cnt = 8 then
							-- ACK bit: release SDA to read ACK
							bsr_data(I2C_SDA_CTRL) <= '1';  -- SDA high-Z
							i2c_return_state <= I2C_CLOCK_HIGH;
							state <= I2C_BSR_SHIFT;
						else
							-- Data bit: drive SDA
							bsr_data(I2C_SDA_CTRL) <= '0';  -- SDA drive
							if i2c_cur_byte(7 - to_integer(i2c_bit_cnt(2 downto 0))) = '1' then
								bsr_data(I2C_SDA_OUT) <= '1';
								bsr_data(I2C_SDA_CTRL) <= '1';  -- high = release
							else
								bsr_data(I2C_SDA_OUT) <= '0';
							end if;
							i2c_return_state <= I2C_CLOCK_HIGH;
							state <= I2C_BSR_SHIFT;
						end if;
					...
					--------------------------------------------------------
					-- I2C CLOCK HIGH: raise SCL, sample if needed
					--------------------------------------------------------
					when I2C_CLOCK_HIGH =>
						bsr_data(I2C_SCL_CTRL) <= '1';  -- SCL high (release)
						i2c_return_state <= I2C_READ_ACK;
						state <= I2C_BSR_SHIFT;

					--------------------------------------------------------
					-- I2C READ ACK: check ACK bit, advance to next bit/byte
					--------------------------------------------------------
					when I2C_READ_ACK =>
						if i2c_bit_cnt = 8 then
							-- This was ACK bit
							if bsr_data(I2C_SDA_IN) = '0' then
								i2c_ack_ok <= '1';  -- ACK received
							else
								i2c_ack_ok <= '0';  -- NACK
							end if;
							i2c_bit_cnt <= (others => '0');
							...
							if i2c_byte_cnt = 0 then
								state <= I2C_STOP_1;  -- Done, send STOP
							else
								i2c_byte_cnt <= i2c_byte_cnt - 1;
								i2c_cur_byte <= i2c_data_buf(to_integer(i2c_byte_cnt - 1) * 8 + 7 downto to_integer(i2c_byte_cnt - 1) * 8);
								state <= I2C_CLOCK_LOW;
							end if;
						else
					...
					--------------------------------------------------------
					-- I2C STOP: SCL low, SDA low, SCL high, SDA high
					--------------------------------------------------------
					when I2C_STOP_1 =>
						bsr_data(I2C_SCL_CTRL) <= '0';  -- SCL drive low
						bsr_data(I2C_SCL_OUT) <= '0';
						bsr_data(I2C_SDA_CTRL) <= '0';  -- SDA drive low
						bsr_data(I2C_SDA_OUT) <= '0';
						i2c_return_state <= I2C_STOP_2;
						state <= I2C_BSR_SHIFT;

					when I2C_STOP_2 =>
						bsr_data(I2C_SCL_CTRL) <= '1';  -- SCL high (release)
						i2c_return_state <= I2C_STOP_3;
						state <= I2C_BSR_SHIFT;

					when I2C_STOP_3 =>
						bsr_data(I2C_SDA_CTRL) <= '1';  -- SDA high (release)
						i2c_return_state <= SEND_RESPONSE_BYTE;
						-- Return ACK status
						resp_buf(7 downto 0) <= "0000000" & i2c_ack_ok;
                            ...
                            if tck_reg = '1' then
                                case to_integer(jtag_cnt) is
                                    ...
                                    when 3 => 
											...
											ir_data <= ir_value;  -- ADD: Save IR value
											tdi_reg <= ir_value(0);
											state <= JTAG_SHIFT_IR;
                                    when others => null;
                                end case;
						...
						if jtag_div = 0 then
							...
							if tck_reg = '1' then
								...
								if shift_cnt = 1 then
									...
									tdi_reg <= ir_data(IR_LENGTH - 1);  -- ADD: Output final bit!
									state <= JTAG_EXIT_IR;
								else
									tdi_reg <= ir_data(to_integer(IR_LENGTH - shift_cnt));
								end if;
							end if;
						end if;
						...
						when JTAG_SHIFT_DR =>
							if jtag_div = 0 then
								...
								if tck_reg = '0' then
									...
									-- Falling edge: store sampled TDO, output next TDI
									if first_bit = '1' then
										first_bit <= '0';  -- Skip storing first sample
									else
										shift_reg(to_integer(dr_len - shift_cnt)) <= tdo_sample;
										shift_cnt <= shift_cnt - 1;
										if shift_cnt = 1 then
											tms_reg <= '1';
											state <= JTAG_EXIT_DR;
										else
											tdi_reg <= tdi_data(to_integer(dr_len - shift_cnt + 1));
										end if;
									end if;
								end if;
							end if;
